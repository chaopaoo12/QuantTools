...
from QUANTTOOLS.QAStockETL.QAFetch import QA_fetch_get_stock_divyield
import pandas as pd
import pymongo
...
def QA_SU_save_stock_divyield_day(client=DATABASE, ui_log = None, ui_progress = None):
    ...
    stock_divyield.create_index([("a_stockcode", pymongo.ASCENDING), ("report_date", pymongo.ASCENDING), ("reg_date", pymongo.ASCENDING)], unique=True)
    err = []
...
def QA_SU_save_stock_divyield_his(client=DATABASE, ui_log = None, ui_progress = None):
    ...
    date_list = list(pd.DataFrame.from_dict(QA_util_getBetweenQuarter(START_DATE,END_DATE)).T.iloc[:,1])
    stock_divyield = client.stock_divyield
    stock_divyield.create_index([("a_stockcode", pymongo.ASCENDING), ("date_stamp", pymongo.ASCENDING), ("reg_date", pymongo.ASCENDING)], unique=True)
    err = []
