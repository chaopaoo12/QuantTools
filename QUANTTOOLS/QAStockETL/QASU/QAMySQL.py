...
def pct(data):
    ...
    data['RNG_10']= ((data['high_qfq'].rolling(window=10).max()-data['low_qfq'].rolling(window=10).min())/data['low_qfq'].rolling(window=5).min()).apply(lambda x:round(x * 100 ,2))
...
def QA_etl_stock_day(type = "day", mark_day = str(datetime.date.today())):
    ...
    if type == "all":
        ...
        data = QA_ETL_stock_day(codes, mark_day, mark_day).reset_index()
        if data is None:
