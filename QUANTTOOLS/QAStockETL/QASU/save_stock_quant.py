...
def QA_SU_save_stock_quant_day(code=None, start_date=None,end_date=None, ui_log = None, ui_progress = None):
    ...
    if code is None:
        timeToMarket = pd.DataFrame(QA_fetch_stock_basic_info_tushare()).set_index('code')[['name', 'timeToMarket']]
        timeToMarket = timeToMarket.assign(days=timeToMarket.timeToMarket.apply(lambda x:QA_util_get_days_to_today(str(x))))
        code = list(timeToMarket[timeToMarket.days >= 90].index)
        ...
        for deal_date in deal_date_list:
            ...
            if data is not None:
                ...
                financial_data = data[['code','date','date_stamp','INDUSTRY','TOTAL_MARKET', 'TRA_RATE', 'DAYS',
                                       ...
                                       'PE_10PCT','PE_10VAL','PEEGL_10PCT','PEEGL_10VAL','PB_10PCT','PB_10VAL','PEG_10PCT','PEG_10VAL','PS_10PCT','PS_10VAL',
...
def QA_SU_save_index_quant_day(code=None, start_date=None, end_date=None, ui_log = None, ui_progress = None):
    ...
    try:
        ...
        data1 = None
        QA_util_log_info(
            '##JOB got Data index quant data failed ============== from {from_} to {to_} '.format(from_=start_date,to_=end_date), ui_log)
