...
def QA_fetch_get_index_quant_min(code, start_date, end_date, type='30min'):
    if len(code) > 1:
        res = [QA_fetch_get_index_indicator_realtime(i, start_date=start_date, end_date=end_date, type=type) for i in code]
        return(pd.concat(res))
    else:
...
def QA_fetch_get_stock_quant_min(code, start_date, end_date, type='30min', keep = False):
    ...
    while k <= 5:
        try:
            ...
            k += 1
            QA_util_log_info('##JOB QA_fetch_get_stock_quant_min failed with {k} times ===================='.format(k=k), ui_log=None)
            time.sleep(5)
...
def QA_fetch_get_stock_vwap_min(code, start_date, end_date, type='1'):
    if isinstance(code, list):
        if len(code) > 10:
            code = [code[i:i+10] for i in range(0, len(code), 10)]
        ...
        return(QA_fetch_get_stock_vwap(code, start_date, end_date, period = type, type='real'))
