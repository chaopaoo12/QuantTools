...
def pct(data, type = 'close'):
    ...
    if type == 'close':
        data[['PRE_MARKET','high_mark','low_mark']]= data.shift(-1)[['close_qfq','high_qfq','low_qfq']]
        data[['PRE_DATE','PRE2_MARKET']]= data.shift(-2)[['date','close_qfq']]
        data[['PRE3_MARKET']]= data.shift(-3)[['close_qfq']]
        data[['PRE4_MARKET']]= data.shift(-4)[['close_qfq']]
        data[['PRE5_MARKET']]= data.shift(-5)[['close_qfq']]
        data[['PRE10_MARKET']]= data.shift(-10)[['close_qfq']]
        data[['PRE20_MARKET']]= data.shift(-20)[['close_qfq']]
        ...
        data[['PRE_MARKET']]= data.shift(-1)[['close_qfq']]
        data[['PRE2_MARKET']]= data.shift(-2)[['high_qfq']]
        data[['PRE3_MARKET']]= data.shift(-3)[['high_qfq']]
        data[['PRE4_MARKET']]= data.shift(-4)[['high_qfq']]
        data[['PRE5_MARKET']]= data.shift(-5)[['high_qfq']]
        data[['PRE10_MARKET']]= data.shift(-10)[['high_qfq']]
        data[['PRE20_MARKET']]= data.shift(-20)[['high_qfq']]
        data['PASS_MARK'] = (data['PRE_MARKET']/data['close_qfq']-1).apply(lambda x:round(x * 100,2))
        data['TARGET'] = (data['PRE2_MARKET']/data['PRE_MARKET']-1).apply(lambda x:round(x * 100,2))
        data['TARGET3'] = (data['PRE3_MARKET']/data['PRE_MARKET']-1).apply(lambda x:round(x * 100,2))
        data['TARGET4'] = (data['PRE4_MARKET']/data['PRE_MARKET']-1).apply(lambda x:round(x * 100,2))
...
def index_pct(market):
    ...
    market['PASS_MARK'] = (market['PRE_MARKET']/market['close']-1).apply(lambda x:round(x * 100,2))
...
def pct_log(data, type = 'close'):
    ...
    if type == 'close':
        ...
        data['DW_PRICE'] = data['close_qfq'] - (data['close_qfq'] * data['up_rate']).apply(lambda x:round(x,2))
        data['OPEN_MARK'] = ((data['PRE_MARKET'] <= data['DW_PRICE']) | (data['PRE_MARKET'] >= data['UP_PRICE']) | (data['OPEN_MARK'] == 1))*1
        data['PASS_MARK'] = np.log(data['PRE_MARKET']/data['close_qfq'])
...
def index_pct_log(market):
    ...
    market['PRE5_MARKET']= market.shift(-5)['close']
    market['PRE10_MARKET']= market.shift(-10)['close']
    market['PASS_MARK'] = np.log(market['PRE_MARKET']/market['close'])
...
def min_pct(data, type = 'close'):
    ...
    if type == 'close':
        ...
        data[['PRE_MARKET','high_mark','low_mark']]= data.shift(-1)[['close_qfq','high_qfq','low_qfq']]
        data[['PRE2_MARKET']]= data.shift(-2)[['close_qfq']]
        data[['PRE3_MARKET']]= data.shift(-3)[['close_qfq']]
        data[['PRE4_MARKET']]= data.shift(-4)[['close_qfq']]
        data[['PRE5_MARKET']]= data.shift(-5)[['close_qfq']]
        data[['PRE10_MARKET']]= data.shift(-10)[['close_qfq']]
...
def min_pct_log(data, type = 'close'):
    data = data.sort_values('datetime',ascending=True)
