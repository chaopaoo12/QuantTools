...
def QA_util_etl_stock_quant(deal_date = None):
    ...
    sql = '''select a.code,
       ...
       operatingRinrate,
       operatingRinrate_ly,
       operatingRinrate_l2y,
       operatingRinrate_l3y,
       netProfit_inrate,
       netProfit_inrate_ly,
       netProfit_inrate_l2y,
       netProfit_inrate_l3y,
       netCashOperatinrate,
       netCashOperatinrate_ly,
       netCashOperatinrate_l2y,
       netCashOperatinrate_l3y,
       totalProfitinrate,
       totalProfitinrate_ly,
       totalProfitinrate_l2y,
       totalProfitinrate_l3y,
       PT,
       PM,
       PS,
       PC,
  ...
  from (select code,
               ...
               round(case
                     ...
                     2) as PC,
               round(case
                       when NETPROFIT_TTM_LY = 0 or
                            NETPROFIT_TTM = NETPROFIT_TTM_LY then
                        0
                       else
               ...
               round((case
                     ...
                     2) as avg60,
               round(1 -
                     (rank() over(partition by code order by pe desc)) / 61,
                     2) as pe_rank,
               round(1 -
                     (rank() over(partition by code order by pb desc)) / 61,
                     2) as pb_rank
 ...
 left join (select CODE,
    ...
    if deal_date is None:
        ...
        if QA_util_if_trade(deal_date) == True:
            print(deal_date)
            sql = sql.format(start_date=deal_date, b_date = QA_util_get_pre_trade_date(deal_date, 60))
