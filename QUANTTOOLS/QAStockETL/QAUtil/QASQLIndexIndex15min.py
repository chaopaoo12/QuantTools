...
def QA_Sql_Index_Index15min(from_ , to_, type = 'day', sql_text = sql_text, ui_log= None):
    ...
    data = data.assign(SKDJ_TR_15M = (data.SKDJ_CROSS1_15M*-1+ data.SKDJ_CROSS2_15M*1)/(data.SKDJ_CROSS1_15M+data.SKDJ_CROSS2_15M),
                       SHORT_TR_15M = (data.SHORT20_15M > 0)*1,
                       LONG_TR_15M = (data.LONG60_15M > 0)*1)
    data.SKDJ_TR_15M = data.SKDJ_TR_15M.groupby('code').fillna(method='ffill')
    return(data)