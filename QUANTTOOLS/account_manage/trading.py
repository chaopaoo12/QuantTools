...
def build(target, positions, sub_accounts, trading_date, percent, exceptions):
    if target is None:
        ...
        close = QA_fetch_stock_day_adv(list(positions.set_index('证券代码').index),trading_date,trading_date).to_qfq().data.loc[trading_date].reset_index('date')['close']
        ...
        if res[res['mark'] > 0].shape[0] == 0:
            ...
            for i in res[res['mark'] > 0].index:
                if type == 'end':
                    ...
                    INDUSTRY = res.at[i, 'INDUSTRY']
                    mark = abs(float(res.at[i, 'mark']))
                    price = round(float(res.at[i, 'close']*(1-0.0985)),2)
