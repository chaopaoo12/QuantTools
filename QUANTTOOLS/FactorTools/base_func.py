...
def get_quant_data(start_date, end_date):
    ...
    alpha = QA_fetch_stock_alpha_adv(list(QA_fetch_stock_list_adv()['code']),start_date,end_date).data[['alpha_001', 'alpha_002', 'alpha_003', 'alpha_004', 'alpha_005', 'alpha_006', 'alpha_007', 'alpha_008',
                                                                                                        ...
                                                                                                        'alpha_187', 'alpha_188', 'alpha_189', 'alpha_191']].fillna(method='ffill').groupby('code').apply(series_to_supervised,[30,10,7,5,3,1])
    technical = QA_fetch_stock_technical_index_adv(list(QA_fetch_stock_list_adv()['code']),start_date,end_date).data.fillna(method='ffill').groupby('code').apply(series_to_supervised,[10,7,5,4,3,2,1])
    ...
    fianacial = fianacial[cols].groupby('code').apply(series_to_supervised,[30,10,7,5,3,1]).join(fianacial[['GROSSMARGIN', 'GROSSMARGIN_L2Y', 'GROSSMARGIN_L3Y', 'GROSSMARGIN_L4Y', 'GROSSMARGIN_LY',
                                                                                                            ...
                                                                                                            'ROE', 'ROE_L2Y', 'ROE_L3Y', 'ROE_L4Y', 'ROE_LY',
                                                                                                            'TARGET', 'TARGET3', 'TARGET5','INDUSTRY','TOTAL_MARKET']])
    fianacial = fianacial[[x for x in list(fianacial.columns) if x not in ['TARGET', 'TARGET3', 'TARGET5','INDUSTRY','TOTAL_MARKET']]].join(fianacial[['TARGET', 'TARGET3', 'TARGET5','INDUSTRY','TOTAL_MARKET']])
