...
def aotu_report(trading_date):
    ...
    for code in BTC:
        res = btc_daily(code)
        BTC_RES = BTC_RES.append({'code':code, 'daily':res[0], 'weekly':res[1], '日线斜率':res[2], '斜率变动':res[3], '五日线偏离幅度':res[4], '十五日线偏离幅度':res[5], 'SKDJ_MARK':res[6], 'CCI_MARK':res[7]}, ignore_index=True)
