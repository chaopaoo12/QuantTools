...
def load_data(model, start_date, end_date, type ='model', norm_type=None, sub_block=False, ST=False, ui_log=None, code=None):
    ...
    model.get_data(start=QA_util_get_last_day(QA_util_get_real_date(start_date), 30), end= end_date, type =type,sub_block=sub_block, norm_type=norm_type, ST=ST, code=code)
    return(model)
