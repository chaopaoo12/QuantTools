...
def load_data(model, date, k = 3, start = "-01-01", norm_type=None, ui_log = None):
    ...
    model.get_data(start=str(int(date[0:4])-k)+start, end= date, norm_type=norm_type )
    return(model)

def prepare_data(model, date, mark = 0.3, col = 'TARGET5', type = 'percent', k = 3, start = "-01-01", shift = 5, ui_log = None ):
