...
def track_roboot(target_tar, account, trading_date, percent, strategy_id,  exceptions = None):
    ...
    while tm < int(time.strptime("09:30:00", "%H:%M:%S").strftime("%H%M%S")):
        time.sleep(15)
        tm = int(datetime.datetime.now().strftime("%H%M%S"))
    ...
    while tm < int(time.strptime(afternoon_end, "%H:%M:%S").strftime("%H%M%S")):
        ...
        if res is not None:
            if tm >= int(time.strptime(mark_tm, "%H:%M:%S").strftime("%H%M%S")):
        ...
        if tm > int(time.strptime(morning_end, "%H:%M:%S").strftime("%H%M%S")):
            mark = 0
        ...
        while tm > int(time.strptime(morning_end, "%H:%M:%S").strftime("%H%M%S")) and tm < int(time.strptime(afternoon_begin, "%H:%M:%S").strftime("%H%M%S")):
            time.sleep(60)
            tm = int(datetime.datetime.now().strftime("%H%M%S"))
    ...
    if tm > int(time.strptime(afternoon_begin, "%H:%M:%S").strftime("%H%M%S")):
        ###time out
