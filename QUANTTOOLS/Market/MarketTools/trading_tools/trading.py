    ...
    while tm <= int(time.strftime("%H%M%S",time.strptime(afternoon_end, "%H:%M:%S"))):
        ...
        if tm >= int(time.strftime("%H%M%S",time.strptime(mark_tm, "%H:%M:%S"))):
            ...
            if mark_tm in ontm_list or source_data is None:
                ...
                source_data = half_data.join(hour_data[['SKDJ_K_HR','SKDJ_TR_HR','SKDJ_CROSS2_HR','SKDJ_CROSS1_HR','CROSS_JC_HR','CROSS_SC_HR','MA5_HR','MA10_HR','MA60_HR','CCI_HR','CCI_CROSS1_HR','CCI_CROSS2_HR']]).reset_index()
                ...
                if res2 is not None and 'DR' not in name:
                    ...
                    if code in [i for i in list(target_tar.index) if i not in positions.code.tolist()]:
                        ...
                        if res2.SKDJ_CROSS2_HR == 1 and res2.SKDJ_K_30M < 70 and res2.SKDJ_TR_30M > 0 and round(res2.MA5_30M,2) >= 0:
                            ...
                            msg = 'MACD金叉'
                        elif res2.SKDJ_CROSS2_30M == 1 and res2.SKDJ_TR_HR < 70 and res2.SKDJ_TR_HR > 0 and round(res2.MA5_30M,2) >= 0:
