    ...
    while tm <= int(time.strftime("%H%M%S",time.strptime(afternoon_end, "%H:%M:%S"))):
        ...
        if tm >= int(time.strftime("%H%M%S",time.strptime(mark_tm, "%H:%M:%S"))):
            if mark_tm in ontm_list or source_data is None:
                ...
                hour_data = data[['SKDJ_K_30M','SKDJ_TR_30M','SKDJ_K_HR','SKDJ_TR_HR','SKDJ_CROSS2_30M','SKDJ_CROSS1_30M','CROSS_JC_30M','SKDJ_CROSS2_HR','SKDJ_CROSS1_HR','CROSS_JC_HR','CROSS_SC_HR','MA5_HR','MA5_30M','MA10_HR','MA60_HR','CCI_HR','CCI_CROSS1_HR','CCI_CROSS2_HR']]
                source_data = hour_data.groupby('code').fillna(method='ffill').reset_index()
                source_data = source_data[source_data.datetime == stm].set_index('code')
            else:
                QA_util_log_info('##Now Mark Time {},Stm {}, Stock {}'.format(mark_tm,str(stm),len(list(set(positions.code.tolist()+list(target_tar.index))))))
                half_data = get_quant_data_30min(QA_util_get_pre_trade_date(trading_date,10),trading_date,list(set(positions.code.tolist()+list(target_tar.index))), type= 'real')
                source_data = half_data.join(hour_data[['SKDJ_K_HR','SKDJ_TR_HR','SKDJ_CROSS2_HR','SKDJ_CROSS1_HR','CROSS_JC_HR','CROSS_SC_HR','MA5_HR','MA10_HR','MA60_HR','CCI_HR','CCI_CROSS1_HR','CCI_CROSS2_HR']]).groupby('code').fillna(method='ffill').reset_index()
            ...
            for code in positions.code.tolist() + target_list:
                ...
                if res2 is not None and 'DR' not in name:
                    ...
                    if code in positions[positions['可用余额'] > 0].code.tolist():
                        ...
                        if res2.SKDJ_CROSS1_30M == True and round(res2.MA5_30M,2) < 0:
                            msg = 'SKDJ死叉'
                        #elif res2.MA10_HR < 0:
                        #    msg = '打穿MA10'
                    ...
                    if code in [i for i in list(target_tar.index) if i not in positions.code.tolist()]:
                        ...
                        if res2.SKDJ_CROSS2_HR == 1 and res2.SKDJ_K_HR < 70 and res2.SKDJ_TR_30M > 0 and round(res2.MA5_30M,2) > 0:
                            msg = 'SKDJ:30M金叉 小时线K:{}'.format(res2.SKDJ_K_HR)
                        elif res2.CROSS_JC_HR == 1 and res2.SKDJ_K_HR < 70 and res2.SKDJ_TR_30M > 0 and round(res2.MA5_30M,2) > 0:
                            msg = 'MACD金叉'
