...
def predict_target(trading_date, working_dir=working_dir):
    ...
    data = get_quant_data(QA_util_get_pre_trade_date(trading_date,5),trading_date,type='crawl', block=False, sub_block=False,norm_type=None)
    pe_list = data[(data.NETPROFIT_INRATE > 50)&(data.ROE_TTM >= 10)]
    pe_list = prediction_tar.loc[pe_list.index]
    ...
    base_report(trading_date, '模型汇总报告', **{'本日选股': target_pool[((target_pool.RSI3 < 20)|(target_pool.RSI3 > target_pool.RSI2))&(target_pool.ATRR >= 0.03)].sort_values('RANK').head(50),
                                           'INDEX选股': rrr[(rrr.y_pred == 1)&(rrr.RSI3 > rrr.RSI2)&(target_pool.ATRR >= 0.03)],
                                           'PE选股': pe_list[(pe_list.y_pred == 1)&(pe_list.RSI3 > pe_list.RSI2)&(target_pool.ATRR >= 0.03)],
                                           #'进场信号':in_list,
                                           #'出场信号':out_ist
    })