...
def summary_func(trading_date):
    ...
    xg = xg.join(stock_res['RRNG']).assign(model='xg')
    xg_nn = xg_nn.join(stock_res).assign(model='xg_nn')
    mars_nn = mars_nn.join(stock_res).assign(model='mars_nn')
    mars_day = mars_day.join(stock_res['RRNG']).assign(model='mars_day')
    xg_sh = xg_sh.join(stock_res['RRNG']).assign(model='xg_sh')
