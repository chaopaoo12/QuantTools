...
def summary_func(trading_date):
    ...
    xg = stock_res['RRNG'].join(xg).assign(model='xg')
    xg_nn = stock_res.join(xg_nn).assign(model='xg_nn')
    mars_nn = stock_res.join(mars_nn).assign(model='mars_nn')
    mars_day = stock_res['RRNG'].join(mars_day).assign(model='mars_day')
    xg_sh = stock_res['RRNG'].join(xg_sh).assign(model='xg_sh')
