...
def predict_watch(trading_date, working_dir=working_dir):
    ...
    pe_list = data[(data.ROE_RATE > 1)&(data.PE_RATE < 1)&(data.NETPROFIT_INRATE > 50)&(data.ROE_TTM >= 15)&(data.PE_TTM <= 30)&(data.STOCK_TYPE <= 2)][['SKDJ_K_WK','SKDJ_TR_WK','SKDJ_K','SKDJ_TR','SKDJ_K_HR','SKDJ_TR_HR','INDUSTRY','PASS_MARK','TARGET','TARGET3','TARGET4','TARGET5','TARGET10']]
...
def index_predict_watch(trading_date, working_dir=working_dir):
    ...
    kk = prediction_tar.loc[(trading_date,find_stock(rr1[rr1.SKDJ_K_HR <= 40].code.tolist())),].sort_values('SKDJ_K')
