...
def data_collect(code_list,trading_date,data_15min,k_per=1.03):
    try:
        ...
        #底部追涨
        data.loc[(data.VAMPC_K >= 0) & (data.VAMP_K > 0.02) & (data.DISTANCE < 0.02) & (data.camt_vol > 0.8), "signal"] = 1
        data.loc[(data.VAMPC_K >= 0) & (data.VAMP_K > 0.02) & (data.DISTANCE < 0.02) & (data.camt_vol > 0.8), "msg"] = '追涨:VMAP上升通道'
