...
def data_collect(code_list, trading_date, day_temp_data, sec_temp_data, source_data, position, mark_tm, proxies):
    ...
    data.loc[(data.price >= data.BOLL_15M_V)&(data.price >= data.BOLL_30M_V)&(data.stock_chose == 1)&
             ((data.UB_5M_S2 < 0)&(data.UB_5M_S < 0)&(data.UB_5M > 0))&
             ((data.BOLL_15M_S2 < 0)&(data.BOLL_15M_S < 0))&
             ((data.BOLL_30M_S2 < 0)&(data.BOLL_30M_S < 0)), "signal"] = 1
    data.loc[(data.price >= data.BOLL_15M_V)&(data.price >= data.BOLL_30M_V)&(data.stock_chose == 1)&
             ((data.UB_5M_S2 < 0)&(data.UB_5M_S < 0)&(data.UB_5M > 0))&
             ((data.BOLL_15M_S2 < 0)&(data.BOLL_15M_S < 0))&
             ((data.BOLL_30M_S2 < 0)&(data.BOLL_30M_S < 0)), "msg"] = '5M反转进场信号'

    data.loc[(data.price >= data.BOLL_15M_V)&(data.price >= data.BOLL_30M_V)&(data.stock_chose == 1)&
             ((data.UB_5M_S2 < 0)&(data.UB_5M_S < 0)&(data.UB_5M > 0)), "signal"] = 1
    data.loc[(data.price >= data.UB_15M)&(data.price >= data.UB_30M)&(data.stock_chose == 1)&
...
def code_select(target_list, position, day_temp_data, sec_temp_data, trading_date, mark_tm, proxies):
    ...
    sec_temp_data = [res5.join(res15).join(res30).groupby('code').fillna(method='ffill').join(stock_chose[['stock_chose','INDEX_NAME']]).drop_duplicates()]
