...
def signal(buy_list, tmp_list, position, trading_date, mark_tm):
    ...
    if source_data is not None:
        ...
        if time_check_after('09:35:00') is True:
            ...
            data.loc[(data.VAMPC_K >= 0.2) & (data.SKDJ_K_HR <= 20) & (data.DISTANCE < 0.02) & (data.VAMP > data.yes_close * 0.95), "signal"] = 1
            data.loc[(data.VAMPC_K >= 0.2) & (data.SKDJ_K_HR <= 20) & (data.DISTANCE < 0.02) & (data.VAMP > data.yes_close * 0.95), "msg"] = '早盘追涨:水线上VMAP上升通道'

            data.loc[(data.VAMPC_K >= 0.2) & (data.SKDJ_K_HR <= 20) & (data.DISTANCE < 0.02) & (data.VAMP < data.yes_close), "signal"] = 1
            data.loc[(data.VAMPC_K >= 0.2) & (data.SKDJ_K_HR <= 20) & (data.DISTANCE < 0.02) & (data.VAMP < data.yes_close), "msg"] = '早盘追涨:水线下VMAP上升通道'
