...
def data_collect(code_list,trading_date,data_15min,k_per=1.01):
    try:
        ...
        #底部金叉
        data.loc[(data.RRNG_15M.abs() < 0.03)&(data.VAMP_JC == 1) & (data.CLOSE_K > 0) & (data.MIN_V_15M > data.close) & (data.camt_vol > 1),
                 "signal"] = 1
        data.loc[(data.RRNG_15M.abs() < 0.03)&(data.VAMP_JC == 1) & (data.CLOSE_K > 0) & (data.MIN_V_15M > data.close) & (data.camt_vol > 1),
                 "msg"] = 'VMAP金叉'

        #超跌
        data.loc[(data.DISTANCE < -0.03) & (data.VAMP_K > -0.03) & (data.CLOSE_K > 0) & (data.MIN_V_15M * k_per > data.close) & (data.camt_vol > 1),
                 "signal"] = 1
        data.loc[(data.DISTANCE < -0.03) & (data.VAMP_K > -0.03) & (data.CLOSE_K > 0) & (data.MIN_V_15M * k_per > data.close) & (data.camt_vol > 1),
                 "msg"] = 'VMAP超跌'

        #底部追涨
        data.loc[(data.VAMPC_K >= 0.2) & (data.MIN_V_15M * k_per > data.close) & (data.DISTANCE < 0.01) & (data.camt_vol > 1), "signal"] = 1
        data.loc[(data.VAMPC_K >= 0.2) & (data.MIN_V_15M * k_per > data.close) & (data.DISTANCE < 0.01) & (data.camt_vol > 1), "msg"] = '追涨:VMAP上升通道'
...
def signal(target_list, buy_list, position, tmp_data, trading_date, mark_tm):
    ...
    if data is not None:
        ...
        QA_util_log_info(data[['camt_vol','SIGN_30M','RRNG_30M','VAMP_JC','VAMP_SC','VAMP_K','CLOSE_K','VAMPC_K','DISTANCE',
