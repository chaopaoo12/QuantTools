...
def trading_new(trading_date, working_dir=working_dir):
    try:
        ...
        prediction_nn = prediction_tar.loc[QA_util_get_pre_trade_date(trading_date,1)]

        prediction_day = prediction_tar1.loc[QA_util_get_pre_trade_date(trading_date,1)]

        code_list = list(set(prediction_nn[prediction_nn.y_pred == 1].reset_index().code.unique().tolist()
        + prediction_day[prediction_day.y_pred == 1].reset_index().code.unique().tolist()
        ...
        #                     + prediction_tar1[prediction_tar1.y_pred == 1].loc[QA_util_get_pre_trade_date(trading_date,1)].reset_index().code.unique().tolist()
        #                     #+ prediction_tar2[prediction_tar2.RANK <= 20].loc[QA_util_get_pre_trade_date(trading_date,1)].reset_index().code.unique().tolist()
        #                     #+ prediction_tar3[(prediction_tar3.y_pred == 1) & prediction_tar3.TARGET3.isnull()].reset_index().code.unique().tolist()
        #                     ))
