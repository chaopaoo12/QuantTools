...
def trading_new(trading_date, working_dir=working_dir):
    ...
    xg_sh = xg_sh[(xg_sh.RANK <= 20)&(xg_sh.BOLL_V>0)&(xg_sh.TARGET5.isnull())]
    mars_day = mars_day[(mars_day.RANK <= 20)&(mars_day.BOLL_V>0)&(mars_day.TARGET5.isnull())&(mars_day.BOLL_V>0)]
    xg = xg[(xg.RANK <= 20)&(xg.BOLL_V>0)&(xg.TARGET5.isnull())&(xg.BOLL_V>0)]
    xg_nn = xg_nn[(xg_nn.RANK <= 20)&(xg_nn.BOLL_V>0)&(xg_nn.TARGET5.isnull())]
    mars_nn = mars_nn[(mars_nn.RANK <= 20)&(mars_nn.BOLL_V>0)&(mars_nn.TARGET5.isnull())]
