...
def predict(trading_date, strategy_id='机器学习1号', account='name:client-1', working_dir=working_dir, ui_log = None, exceptions=exceptions):
    ...
    save_prediction({'date': trading_date, 'tar':stock_tar, 'stock_tar':stock_tar}, 'prediction_real', working_dir)
    ...
    try:
        stock_res = stock_tar[stock_tar['RANK']<=top].loc[trading_date][['NAME','INDUSTRY','Z_PROB','O_PROB','RANK']]
    except:
