...
def BUY(client, account, strategy_id, account_info, trading_date,
        code, name, industry, deal_pos, target_pos, target, close, type = 'end', test = False):

    real_pos = get_StockPos(code, client, account)
    QA_util_log_info('##JOB Get Real Time Postition {real_pos} Before {code} Buying ===== {date}'.format(
        real_pos=real_pos, code=code, date=trading_date), ui_log=None)

    if type == 'end':
        price = round(QA_fetch_get_stock_realtm_bid(code)+0.01, 2)
        deal_pos = math.floor(round(target / price,0)/100) * 100
        QA_util_log_info('##JOB Get Real Time Price {price} Before {code} Buying ===== {date}'.format(
            price=price, code=code, date=trading_date), ui_log=None)
        ...
        if (price * deal_pos) > UseCapital:
            QA_util_log_info('##JOB {name}({code}){industry} 资金不足 目标买入{deal_pos}股 预估{target} 实际{capital}===={date}'.format(
                date=trading_date, code=code, name= name, industry=industry, deal_pos=abs(deal_pos),
        ...
        QA_util_log_info('买入 {code}({name},{industry}) {deal_pos}股, 目标持仓:{target_pos},单价:{price},总金额:{target}'.format(
            code=code, name=name, industry=industry, deal_pos=abs(deal_pos), target_pos=deal_pos,
            price=price, target=abs(deal_pos)*price), ui_log=None)
        if test == False:
