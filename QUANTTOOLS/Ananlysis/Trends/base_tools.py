...
def trends_func(func, code, date):
    ...
    data_ind[['mean','per25','per75','perc','percs']] = data_ind['RRNG'].rolling(1800,min_periods=60).agg(['mean', per25, per75, perc, percs])
...
def trends_globalindex(GOLD, date):
    data, week = trends_func1(QA_fetch_get_globalindex_day, GOLD)
    return(data, week)
...
def trends_stock(code, start_date, end_date, period='day', type='before'):
    ...
    data_index[['mean','per25','per75','perc', 'percs']] = data_index['RRNG'].rolling(1800,min_periods = 60).agg(['mean', per25, per75, perc, percs])
