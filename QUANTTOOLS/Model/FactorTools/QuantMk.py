...
def get_quant_data(start_date, end_date, code=None, type = 'crawl', block = False, sub_block= True, method = 'value', norm_type = 'normalization', ST=False, n_in = None):
    ...
    if n_in is None:
        ...
        res = res[[i for i in res.columns if i not in ['INDUSTRY','next_date','PASS_MARK','TARGET',
                                                       'TARGET3','TARGET4','TARGET5','TARGET10','TARGET20']]].groupby('code').apply(series_to_supervised, n_in = n_in).join(res['INDUSTRY','next_date','PASS_MARK','TARGET',
...
def get_quant_data_neut(start_date, end_date, code=None, type = 'crawl', block = False, sub_block= True, method = 'value', ST=True, n_in= None):
    ...
    if n_in is None:
        ...
        res = res[[i for i in res.columns if i not in ['INDUSTRY','next_date','PRE_DATE','PASS_MARK','TARGET',
                                                       'TARGET3','TARGET4','TARGET5','TARGET10','TARGET20']]].groupby('code').apply(series_to_supervised, n_in = n_in).join(res[['INDUSTRY','next_date','PRE_DATE','PASS_MARK','TARGET','TARGET3','TARGET4','TARGET5','TARGET10','TARGET20']])
