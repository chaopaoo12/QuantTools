import pandas as pd
from QUANTTOOLS.Model.FactorTools.QuantMk import get_index_quant_15min
...
class QAIndexModel15Min(QAModel):
    ...
    def get_data(self, start, end, code=None, type ='model', norm_type=None):
        ...
        self.data = get_index_quant_15min(start, end, code=code, type = type, norm_type=norm_type)
        self.info['code'] = code
    ...
    def model_predict(self, start, end, code = None, type='crawl'):
        if code is not None:
            self.code = code
        QA_util_log_info('##JOB Got Index Quant 15min Data by {type}==== from {_from} to {_to} target:{target}'.format(type=type, _from=start, _to=end, target = self.target), ui_log = None)
        data = get_index_quant_15min(start, end, code = self.code, type= type)
        ...
        b = b.join(data[['SKDJ_TR_15M','CCI_15M','SKDJ_CROSS2_15M','SKDJ_CROSS1_15M','CROSS_JC_15M']])
