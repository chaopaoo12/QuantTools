import pandas as pd
from QUANTTOOLS.Model.FactorTools.QuantMk import get_hedge_data,get_hedge_data_realtime
...
class QAStockModelHedgeReal(QAModel):
    ...
    def get_data(self, start, end, code =None, block=True, sub_block=False, type ='crawl', norm_type='normalization'):
        ...
        self.data = get_hedge_data(start, end, code=code, type = type, block = block, sub_block = sub_block, norm_type=norm_type)
    ...
    def model_predict(self, start, end, type='model'):
        ...
        data = get_hedge_data_realtime(start, end, self.code, type= type,block = self.block, sub_block=self.sub_block, norm_type=self.norm_type)
