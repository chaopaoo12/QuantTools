...
class QAIndexModelHour(QAModel):
    ...
    def model_predict(self, start, end, code = None, type='crawl'):
        if code is not None:
            self.code = code
        QA_util_log_info('##JOB Got Index Quant hour Data by {type}==== from {_from} to {_to} target:{target}'.format(type=type, _from=start, _to=end, target = self.target), ui_log = None)
        ...
        b = b.join(data[['SKDJ_TR_HR','SKDJ_K_HR','SKDJ_CROSS2_HR','SKDJ_CROSS1_HR','CROSS_JC_HR']])
