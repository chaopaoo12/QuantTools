...
import QUANTTOOLS.QAStockTradingDay.StockModel.StrategyXgboost as Stock
import QUANTTOOLS.QAIndexTradingDay.IndexModel.IndexXGboost as Index
import pandas as pd
...
def concat_predict(trading_date, strategy_id='机器学习1号',  working_dir=working_dir, ui_log = None):
    ...
    index_tar, index_b  = Index.model_predict(index_model_temp, start, end, index_info_temp['cols'], index_info_temp['thresh'])
    QA_util_log_info(
    ...
    safe_tar, safe_b  = Index.model_predict(safe_model_temp, start, end, safe_info_temp['cols'], safe_info_temp['thresh'])
    QA_util_log_info(
